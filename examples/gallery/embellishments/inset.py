...
# Set the land color to "brown", the water color to "lightblue", and the shorelines width to "thin"
fig.coast(land="brown", water="lightblue", shorelines="thin")

# Create an inset, placing it in the Top Left (TL) corner with a width of 3.5 cm and x- and
# y-offsets of 0.2 cm. The clearance is set to 0, and the border is "gold" with a pen thickness of
# 1.5 points.
with fig.inset(
    ...
    # Create a map within the inset. This example uses the azimuthal orthogonal projection centered
    # at 47 E, 20 S. The question mark is required for the automatic size determination by PyGMT
