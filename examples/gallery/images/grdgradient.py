...
--------------------
The :meth: `pygmt.grdclip` calculate the gradient of a grid file.
In the example shown below we will see how to calculate an hillshade map based on
a Data Elevation Model(DEM). :meth: `pygmt.grdclip` get as input :class: `xarray.DataArray' object
...
pygmt.config(FORMAT_GEO_MAP='ddd.x', MAP_FRAME_TYPE='plain', FONT='9p')
...
fig.grdimage(
    grid=dgrid,
    projection="M12c",
    frame=['WSrt+t"Hillshaded Map"', 'xa0.3', 'ya0.1'],
    cmap=True,
    region=region
)

# Shift plot origin of the second map by "width of the first map + 0.1 cm"
# in x direction
...
fig.grdimage(
    grid=grid,
    projection="M12c",
    frame=['lSrt+t"Original Data Elevation Model"', 'xa0.3', 'ya0.1'],
    cmap=True,
    region=region
)
fig.colorbar(
    position="JMR+o0.2c/0c+w3c/0.3c+mc",
