...
pygmt.config(FONT_TITLE = "15p,5", MAP_TITLE_OFFSET = "10p")
...
with fig.subplot(nrows=3, ncols=4, figsize=("24c", "21c")):
    # Azimuth: define where is the light source. e.g. 0/90 illuminates from
    # the north (top) and east (right), and so on. 
    for i, azi in enumerate(["0/90", "0/180", "0/300"]):
        # Normalize: to enhance the three-dimensional sense of the topography. 
        # It calculates the azimuthal gradient of each point along a certain
        # azimuth angle, then adjust the brightness value of the color 
        # according to the positive/negative of the azimuthal gradient and the
