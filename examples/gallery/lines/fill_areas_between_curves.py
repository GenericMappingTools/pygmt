...
to set different fills for areas with y1 > y2 and y1 < y2, respectively. Use the  ``label``
and ``label2`` parameters to set the corresponding legend entries. In addition to filling
the areas, we can draw the curves. Use the ``pen`` and  ``pen2`` parameters to set different
...
# %%
# To compare two co-registered curves y1(x) and y2(x) pass a sequence with the same length
# as the inputs for ``x`` and ``y`` to ``y2``.
