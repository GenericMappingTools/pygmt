...
# annotation on the colorbar, in this case every 1,000 meters. To set the label for
# an axis on the colorbar, the argument begins with either ``"x+l"`` (x-axis) or
# ``"y+l"`` (y-axis), followed by the intended label.
#
...
# ``grid`` (in this case, the variable named "grid") must be input again. The
# ``levels`` parameter sets the spacing between adjacent contour lines (in this case,
# 500 meters). The ``annotation`` parameter annotates the contour lines corresponding
# to the given interval (in this case, 1,000 meters) with the related values, here
# elevation or bathymetry. By default, these contour lines are drawn thicker.
# Optionally, the appearance (thickness, color, style) of the annotated and the
# not-annotated contour lines can be adjusted (separately) by specifying the desired
# ``pen``.
...
# -------------
#
# To make it clear where the islands are located, the :meth:`pygmt.Figure.coast`
# method can be used to color in the landmasses. The ``land`` is colored in as
# "lightgray", and the ``shorelines`` parameter draws a border around the islands.
...
#    `here <https://www.generic-mapping-tools.org/remote-datasets/earth-relief.html#usage>`_.
#
# 2. Create a contour map of the area around Mt. Rainier. A suggestion for the
#    ``region`` would be ``[-122, -121, 46.5, 47.5]``.
#    Adjust the :meth:`pygmt.Figure.grdcontour` and :meth:`pygmt.Figure.colorbar`
#    settings as needed to make the figure look good.
#
# 3. Create a contour map of São Miguel Island in the Azores; a suggested ``region``
#    is ``[-26, -25, 37.5, 38]``. Instead of coloring in ``land``, set ``water`` to
