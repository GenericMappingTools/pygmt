...
- A grid is a 2D array of data that is regularly spaced in the x and y directions. 
...
# ----------------
#
# Most PyGMT functions/methods that accept table input data has a ``data`` parameter.
# The easiest way to provide table input data to PyGMT is by specifying the
# file name of an ASCII table (e.g., `data="table-input.txt"`). This is useful
...
# -----------------------------------
#
# The `data` parameter also accepts a 2D numpy array or a pandas.DataFrame object.
...
# Pass a 2D numpy array to the `data` parameter
