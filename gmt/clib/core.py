...
from .utils import load_libgmt, check_status_code, kwargs_to_ctypes_array
...
class LibGMT():  # pylint: disable=too-many-instance-attributes
    ...
    def log_to_file(self):
        ...
        status = self._c_handle_messages(self.current_session,
                                         ...
                                         logfile.encode())
        check_status_code(status, 'GMT_Handle_Messages')
    ...
    def call_module(self, module, args):
        ...
        with self.log_to_file() as logfile:
            status = self._c_call_module(self.current_session, module.encode(),
                                         mode, args.encode())
            check_status_code(status, 'GMT_Call_Module', logfile)
