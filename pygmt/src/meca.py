...
def _preprocess_spec(
    ...
    if isinstance(spec, dict):
        if kind != "matrix":
            # For dict/pandas.DataFrame input, the values can be overridden by the
            # parameters given in the function arguments.
            override_cols = {
                "longitude": longitude,
                "latitude": latitude,
                "depth": depth,
...
def _set_offset(spec, offset):
    ...
    If the input data contains ``plot_longitude`` and ``plot_latitude``, then we
    automatically set the offset parameter to ``True``.
    """
