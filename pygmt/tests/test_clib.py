...
def test_fails_for_wrong_version(monkeypatch):
    ...
    with monkeypatch.context() as mpatch:
        ...
        # Reload the clib.sesion module so that patched get_gmt_version is called.
        importlib.reload(clib.session)
        assert clib.__gmt_version__ == "5.4.3"  # Old GMT version is returned.
        with pytest.raises(GMTVersionError):
            # Raise an exception if pygmt.clib is loaded/reloaded.
