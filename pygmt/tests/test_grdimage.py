...
def test_grdimage_over_dateline(xrgrid):
    ...
    ``GMT_GRID_IS_GEO``, and that ``xrgrid.gmt.registration = GridReg.GRIDLINE` sets
    ``GMT_GRID_NODE_REG``. Note that there would be a gap over the dateline if a pixel
    registered grid is used.
    See also https://github.com/GenericMappingTools/pygmt/issues/375.
    """
    fig = Figure()
    assert xrgrid.gmt.registration == GridReg.GRIDLINE
...
def test_grdimage_grid_no_redundant_360():
    ...
    da3 = da1[:, 0:360]
    da3.gmt.registration = GridReg.GRIDLINE
    ...
    da4 = da2[:, 0:360]
    da4.gmt.registration = GridReg.GRIDLINE
